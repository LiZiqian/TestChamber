  ...
  taskSamplePickerSampleRowHtml(sample, state, { selected = false } = {}) {
    ...
    return `
      <div class="dispatch-sample-row ${selectable ? "" : "is-disabled"}" onclick="app.onTaskSampleRowClick(event,'${state.inputName}','${state.progressSelectId}','${state.hintId}')" title="${Utils.esc(disabledReason)}">
        ...
        <div class="dispatch-sample-status"><span class="badge ${this.sampleHasProblem(sample) ? 's-故障' : 's-OK'}">${this.sampleHasProblem(sample) ? '故障' : 'OK'}</span><span class="badge s-${Utils.esc(status)}">${Utils.esc(status)}</span></div>
      </div>`;
  },
