...
def build_index() -> dict[str, dict[str, int]]:
    ...
    for path in ROOT.rglob("*"):
        if not path.is_file() or any(part in {".git", "__pycache__", "data", "backups"} for part in path.parts):
            continue
        if path.suffix == ".py":
...
def update_heading_counts(text: str) -> str:
    server_lines = len((ROOT / "server.py").read_text(encoding="utf-8").splitlines())
    text = re.sub(r"`server\.py` 约 [\d,]+ 行", f"`server.py` 约 {server_lines:,} 行", text)
    text = re.sub(r"### server\.py \(\d+ 行", f"### server.py ({server_lines} 行", text)
    return text
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        if refreshed != text:
            print("AGENTS.md file map is stale; run: python tools\\update_agents_map.py")
            return 1
        return 0
