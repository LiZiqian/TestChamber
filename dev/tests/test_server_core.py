...
from backend.server_modules import http_api  # noqa: E402
...
def call_fake_get(path):
    handler = FakeHttpHandler()
    handler.path = path
    handler.client_address = ("127.0.0.1", 12345)
    handler._send_json = lambda payload, status=200: server.http_helpers.send_json(
        handler,
        payload,
        status=status,
        json_dumps=server.json_dumps,
    )
    handler._send_bytes = lambda data, content_type, status=200, cache="no-store": server.http_helpers.send_bytes(
        handler,
        data,
        content_type,
        status=status,
    ...
    handler._send_file = lambda target, content_type, cache="public, max-age=86400, must-revalidate": server.http_helpers.send_file(
        handler,
        target,
        content_type,
        cache=cache,
    )
    handler._sample_archive_route = server.http_routes.sample_archive_route
    handler._project_detail_route = server.http_routes.project_detail_route
    handler._sample_category_detail_route = server.http_routes.sample_category_detail_route
    handler._stage_tasks_route = server.http_routes.stage_tasks_route
    handler._sample_category_samples_route = server.http_routes.sample_category_samples_route
    handler._sample_photo_route = server.http_routes.sample_photo_route
    handler._sample_events_route = server.http_routes.sample_events_route
    handler._sample_history_route = server.http_routes.sample_history_route
    handler._is_public_static_path = server.http_routes.is_public_static_path
    http_api.handle_get(handler, server.http_runtime_context())
    return handler


class ServerCoreTests(unittest.TestCase):
    def test_data_root_stays_inside_project_data_directory(self):
        current_paths = server._RUNTIME_PATHS
        try:
            configured = server.set_data_root(server.ROOT_DIR / "data")
            self.assertEqual(configured, server.ROOT_DIR / "data")
            with self.assertRaises(ValueError):
                server.set_data_root(server.ROOT_DIR.with_name(f"{server.ROOT_DIR.name}_data"))
        finally:
            server._apply_runtime_paths(current_paths)

    def test_runtime_paths_migrate_legacy_data_without_copying_backups(self):
        with tempfile.TemporaryDirectory() as tmp:
            platform_root = Path(tmp) / "Platform"
            platform_root.mkdir()
            legacy_data = platform_root.with_name("Platform_data")
            ...
            expected_bytes = sum(
                path.stat().st_size
                for path in legacy_data.rglob("*")
                if path.is_file() and "backups" not in path.relative_to(legacy_data).parts
            )

            paths, report = server.runtime_paths.prepare_runtime_paths(platform_root)

            self.assertTrue(report.migrated)
            self.assertEqual(paths.data_dir, platform_root / "data")
            ...
            self.assertFalse((paths.data_dir / "backups").exists())
            ...
            self.assertEqual(marker["dataRoot"], str(paths.data_dir))
            self.assertEqual(marker["migration"]["copiedBytes"], expected_bytes)
            self.assertEqual(marker["migration"]["verifiedFiles"], 3)
            self.assertNotIn("legacyBackupDir", marker["migration"])
    ...
    def test_default_startup_uses_project_internal_data_root(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                server.FRONTEND_DIR = platform_root / "frontend"
                server.DEFAULT_DATA_DIR = server.runtime_paths.default_data_dir(platform_root)
                server.INDEX_PATH = server.FRONTEND_DIR / "index.html"

                report = server.prepare_runtime_data_root()
                server.init_db()

                expected_data_root = platform_root / "data"
                self.assertEqual(server.DATA_DIR, expected_data_root)
                self.assertTrue(server.runtime_paths.path_is_inside(server.DATA_DIR, platform_root))
                ...
                self.assertTrue((platform_root / "data").exists())
    ...
    def test_runtime_paths_skip_migration_when_project_data_has_business_data(self):
        with tempfile.TemporaryDirectory() as tmp:
            platform_root = Path(tmp) / "Platform"
            platform_root.mkdir()
            legacy_data = platform_root.with_name("Platform_data")
            ...
            target_root = platform_root / "data"
            target_root.mkdir()
    ...
    def test_runtime_paths_rolls_back_staging_if_target_changes_during_migration(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual((legacy_data / "testchamber.sqlite").read_bytes(), b"legacy-db")
            leftovers = list(target_root.parent.glob(".data.migration-*"))
    ...
    def test_status_normalization_domains_use_canonical_names(self):
        status = server.status_normalization
        self.assertEqual(status.normalize_task_flow_status({"status": "完成"}), "正常完成")
        self.assertEqual(status.normalize_task_flow_status({"status": "Fail"}), "异常终止")
        self.assertEqual(status.normalize_task_result_value("Fail"), "不通过")
        self.assertEqual(status.normalize_task_result_value("Pass"), "通过")
        stage = status.normalize_stage_payload({
            "id": "st1",
            "progress": [{
                "id": "prog1",
                "strategyId": "strat1",
        ...
        self.assertEqual(status.normalize_sample_usage_status("已借出"), "取走分析")
        self.assertEqual(status.normalize_sample_usage_status("待维修"), "闲置")
        self.assertEqual(status.normalize_sample_quality_value("OK"), "无故障")
        self.assertEqual(status.normalize_sample_quality_value("故障"), "有故障")
        self.assertEqual(status.normalize_display_fallback("变更退出"), "退出测试")
        sample = status.normalize_sample_payload({"id": "testing", "sampleIds": ["Pass"], "sn": "OK", "status": "故障"})
        self.assertEqual(sample["id"], "testing")
        self.assertEqual(sample["sampleIds"], ["Pass"])
        self.assertEqual(sample["sn"], "OK")
        self.assertEqual(sample["status"], "闲置")
        self.assertTrue(sample["hasProblem"])

    def test_status_normalization_backfill_canonicalizes_sqlite_rows(self):
        conn = sqlite3.connect(":memory:")
        conn.row_factory = sqlite3.Row
        server.ensure_schema(conn)
        now = server.now_iso()
        conn.execute(
            "INSERT OR REPLACE INTO project_records (id, name, code, owner, sort_order, data_json, created_at, updated_at, deleted_at) VALUES (?, ?, '', '', 0, ?, ?, ?, NULL)",
            ("p1", "项目", server.json_dumps({"id": "p1", "name": "项目"}), now, now),
        )
        conn.execute(
            "INSERT OR REPLACE INTO project_stages (id, project_id, name, sort_order, data_json, created_at, updated_at, deleted_at) VALUES (?, ?, ?, 0, ?, ?, ?, NULL)",
            ("st1", "p1", "阶段", server.json_dumps({"id": "st1", "projectId": "p1", "progress": [{"id": "prog1", "status": "Testing"}]}), now, now),
        )
        task_json = {
            "id": "task1",
            "status": "阻塞",
            "resultUploads": [{"id": "result1", "result": "Fail", "samples": [{"sampleId": "sample1", "fault": "OK"}]}],
            "logs": [{"id": "log1", "action": "上传 Fail", "reason": "OK，变更退出"}],
            "sampleIds": ["sample1"],
        }
        conn.execute(
            """
            ...
            VALUES (?, ?, ?, ?, '', '', 0, ?, ?, '', ?, ?, ?, ?, '', NULL)
            """,
            ("task1", "p1", "st1", "prog1", "阻塞", "阻塞", server.json_dumps(["sample1"]), server.json_dumps(task_json), now, now),
        )
        conn.execute(
            "INSERT OR REPLACE INTO project_task_samples (task_id, sample_id, project_id, stage_id, test_item, status, flow_status, updated_at) VALUES (?, ?, ?, ?, '', ?, ?, ?)",
            ("task1", "sample1", "p1", "st1", "阻塞", "阻塞", now),
        )
        conn.execute(
            "INSERT OR REPLACE INTO sample_categories (id, name, description, sort_order, data_json, created_at, updated_at, deleted_at) VALUES (?, ?, '', 0, ?, ?, ?, NULL)",
            ("cat1", "池", server.json_dumps({"id": "cat1", "name": "池"}), now, now),
        )
        conn.execute(
            """
            ...
            VALUES (?, ?, '', '', '', '', 0, ?, 0, ?, '', '', '', ?, ?, ?, NULL)
            """,
            ("sample1", "cat1", "已借出", "已借出", server.json_dumps({"id": "sample1", "status": "已借出", "problemRecords": [{"description": "故障"}]}), now, now),
        )
        conn.execute(
            """
            INSERT OR REPLACE INTO sample_records
            ...
            VALUES (?, ?, '', '', '', '', 0, ?, 0, ?, '', '', '', ?, ?, ?, NULL)
            """,
            ("sample2", "cat1", "闲置", "故障", server.json_dumps({"id": "sample2", "status": "闲置"}), now, now),
        )
        conn.execute(
            "INSERT OR REPLACE INTO task_logs (id, task_id, project_id, stage_id, time, action, user, data_json) VALUES (?, ?, ?, ?, ?, ?, '', ?)",
            ("log1", "task1", "p1", "st1", now, "上传 Fail", server.json_dumps({"id": "log1", "reason": "Pass OK 故障"})),
        )
        conn.execute(
            "INSERT OR REPLACE INTO sample_events (id, sample_id, time, event_type, project_id, stage_id, task_id, test_item, user, data_json) VALUES (?, ?, ?, 'sample_log', ?, ?, ?, '', '', ?)",
            ("event1", "sample1", now, "p1", "st1", "task1", server.json_dumps({"id": "event1", "flowStatus": "故障", "reason": "变更退出"})),
        )
        conn.execute(
            "INSERT INTO audit_log (time, user, action, remark, revision_before, revision_after, client_ip) VALUES (?, '', ?, ?, 1, 2, '127.0.0.1')",
            (now, "保存 Fail", "Pass OK"),
        )

        server.ensure_schema(conn)

        task_row = conn.execute("SELECT status, flow_status, data_json FROM project_tasks WHERE id = 'task1'").fetchone()
        self.assertEqual(task_row["status"], "阻塞中")
        self.assertEqual(task_row["flow_status"], "阻塞中")
        self.assertEqual(server.json_obj(task_row["data_json"], {})["resultUploads"][0]["result"], "不通过")
        stage_row = conn.execute("SELECT data_json FROM project_stages WHERE id = 'st1'").fetchone()
        self.assertEqual(server.json_obj(stage_row["data_json"], {})["progress"][0], {"id": "prog1"})
        link_row = conn.execute("SELECT status, flow_status FROM project_task_samples WHERE task_id = 'task1'").fetchone()
        self.assertEqual(link_row["status"], "阻塞中")
        self.assertEqual(link_row["flow_status"], "阻塞中")
        sample_row = conn.execute("SELECT status, effective_status, has_problem, data_json FROM sample_records WHERE id = 'sample1'").fetchone()
        self.assertEqual(sample_row["status"], "取走分析")
        self.assertEqual(sample_row["effective_status"], "取走分析")
        self.assertEqual(int(sample_row["has_problem"]), 1)
        self.assertIn("有故障", server.json_dumps(server.json_obj(sample_row["data_json"], {})))
        sample2_row = conn.execute("SELECT status, effective_status, has_problem, data_json FROM sample_records WHERE id = 'sample2'").fetchone()
        self.assertEqual(sample2_row["status"], "闲置")
        self.assertEqual(sample2_row["effective_status"], "闲置")
        self.assertEqual(int(sample2_row["has_problem"]), 1)
        self.assertIn("有故障", server.json_dumps(server.json_obj(sample2_row["data_json"], {})))
        log_row = conn.execute("SELECT action, data_json FROM task_logs WHERE id = 'log1'").fetchone()
        self.assertEqual(log_row["action"], "上传 不通过")
        self.assertIn("无故障", log_row["data_json"])
        event_row = conn.execute("SELECT data_json FROM sample_events WHERE id = 'event1'").fetchone()
        self.assertIn("退出测试", event_row["data_json"])
        audit_row = conn.execute("SELECT action, remark FROM audit_log ORDER BY id DESC LIMIT 1").fetchone()
        self.assertEqual(audit_row["action"], "保存 不通过")
        self.assertEqual(audit_row["remark"], "通过 无故障")
    ...
    def test_schema_prunes_orphan_operational_logs(self):
        conn = sqlite3.connect(":memory:")
        conn.row_factory = sqlite3.Row
        server.ensure_schema(conn)
        try:
            server.record_writers.update_project_record(conn, {"id": "p1", "name": "项目A"}, create_if_missing=True)
            server.record_writers.update_stage_record(conn, {"id": "st1", "name": "阶段A"}, "p1", "st1", create_if_missing=True)
            server.record_writers.upsert_task_record(
                conn,
                {"id": "task1", "status": "待下发", "sampleIds": ["sample1"], "logs": []},
                "p1",
                "st1",
                create_if_missing=True,
            )
            server.record_writers.update_sample_category_record(conn, {"id": "cat1", "name": "样机池"}, create_if_missing=True)
            server.record_writers.update_sample_record(
                conn,
                {"id": "sample1", "categoryId": "cat1", "sampleNo": "S001", "status": "闲置"},
                create_if_missing=True,
            )
            rows = [
                ("task_logs", ("log_valid", "task1", "p1", "st1", "", "有效", "", "{}")),
                ("task_logs", ("log_missing_task", "missing_task", "p1", "st1", "", "孤儿", "", "{}")),
                ("project_task_samples", ("task1", "sample1", "p1", "st1", "", "", "", "")),
                ("project_task_samples", ("missing_task", "sample1", "p1", "st1", "", "", "", "")),
                ("project_task_samples", ("task1", "missing_sample", "p1", "st1", "", "", "", "")),
            ]
            for table, values in rows:
                placeholders = ",".join("?" for _ in values)
                conn.execute(f"INSERT OR REPLACE INTO {table} VALUES ({placeholders})", values)
            conn.execute(
                """
                INSERT INTO sample_assets
                (id, sample_id, kind, original_name, file_name, relative_path, mime_type, size, created_at, created_by, deleted_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)
                """,
                ("asset_valid", "sample1", "photo", "front.jpg", "front.jpg", "samples/sample1/photos/front.jpg", "image/jpeg", 1, "", ""),
            )
            conn.execute(
                """
                INSERT INTO sample_assets
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)
                """,
                ("asset_orphan", "missing_sample", "photo", "front.jpg", "front.jpg", "samples/missing/photos/front.jpg", "image/jpeg", 1, "", ""),
            )
            server.record_writers.upsert_sample_events(conn, [
                {"id": "event_valid_task", "sampleId": "sample1", "taskId": "task1", "source": "有效任务事件"},
                {"id": "event_valid_sample", "sampleId": "sample1", "source": "有效样机事件"},
                {"id": "event_missing_sample", "sampleId": "missing_sample", "source": "孤儿样机事件"},
                {"id": "event_missing_task", "sampleId": "sample1", "taskId": "missing_task", "source": "孤儿任务事件"},
                {"id": "event_empty_sample", "sampleId": "", "source": "空样机事件"},
            ])
            conn.execute(
                """
                INSERT INTO audit_log (time, user, action, remark, revision_before, revision_after, client_ip)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                ("", "管理员", "test", "审计日志不是孤儿业务履历", 1, 2, "127.0.0.1"),
            )

            server.ensure_schema(conn)

            self.assertEqual(conn.execute("SELECT id FROM task_logs").fetchone()["id"], "log_valid")
            link_rows = conn.execute("SELECT task_id, sample_id FROM project_task_samples").fetchall()
            self.assertEqual([(row["task_id"], row["sample_id"]) for row in link_rows], [("task1", "sample1")])
            self.assertEqual(conn.execute("SELECT id FROM sample_assets").fetchone()["id"], "asset_valid")
            event_ids = [
                row["id"]
                for row in conn.execute("SELECT id FROM sample_events ORDER BY id").fetchall()
            ]
            self.assertEqual(event_ids, ["event_valid_sample", "event_valid_task"])
            self.assertEqual(conn.execute("SELECT COUNT(*) AS count FROM audit_log").fetchone()["count"], 1)
        finally:
            conn.close()

    def test_schema_clears_audit_log_when_platform_is_empty(self):
        conn = sqlite3.connect(":memory:")
        conn.row_factory = sqlite3.Row
        server.ensure_schema(conn)
        try:
            server.record_writers.upsert_sample_events(conn, [
                {"id": "event_orphan", "sampleId": "missing_sample", "source": "孤儿事件"},
            ])
            conn.execute(
                """
                INSERT INTO audit_log (time, user, action, remark, revision_before, revision_after, client_ip)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                ("", "管理员", "delete_everything", "空平台旧审计", 1, 2, "127.0.0.1"),
            )

            server.ensure_schema(conn)

            self.assertEqual(conn.execute("SELECT COUNT(*) AS count FROM sample_events").fetchone()["count"], 0)
    ...
    def test_version_endpoints_and_templates_use_single_version_file(self):
        expected_version = (ROOT / "VERSION").read_text(encoding="utf-8").strip()
        self.assertEqual(server.APP_VERSION, expected_version)
        self.assertEqual(server.SERVER_VERSION, f"TestChamberServer/{expected_version}")

        health = call_fake_get("/api/health")
        health_payload = json.loads(health.wfile.getvalue().decode("utf-8"))
        self.assertEqual(health.status, 200)
        self.assertEqual(health_payload["version"], expected_version)

        with tempfile.TemporaryDirectory() as tmp:
            with patched_server_data_dirs(tmp):
                server.init_db()
                bootstrap = call_fake_get("/api/bootstrap")
                bootstrap_payload = json.loads(bootstrap.wfile.getvalue().decode("utf-8"))
                self.assertEqual(bootstrap.status, 200)
                self.assertEqual(bootstrap_payload["version"], expected_version)

        index = call_fake_get("/")
        index_text = index.wfile.getvalue().decode("utf-8")
        self.assertEqual(index.status, 200)
        self.assertNotIn("__APP_VERSION__", index_text)
        self.assertIn(f'content="{expected_version}"', index_text)
        self.assertIn(f"/css/style.css?v={expected_version}", index_text)
        self.assertIn(f"/js/app.core.js?v={expected_version}", index_text)

        style = call_fake_get("/css/style.css")
        style_text = style.wfile.getvalue().decode("utf-8")
        self.assertEqual(style.status, 200)
        self.assertNotIn("__APP_VERSION__", style_text)
        self.assertIn(f"?v={expected_version}", style_text)
    ...
    def test_selected_sample_export_filters_domain_documents(self):
        data = empty_state()
        data["sampleLibrary"]["categories"] = [{
            "id": "cat_export_scope",
            "name": "范围池",
            "samples": [
                {"id": "sample_keep", "sampleNo": "K001", "sn": "SN-KEEP", "status": "闲置", "photos": []},
                {"id": "sample_drop", "sampleNo": "D001", "sn": "SN-DROP", "status": "闲置", "photos": []},
            ],
        }]
        conn = state_conn(data)
        with tempfile.TemporaryDirectory() as tmp, patched_server_data_dirs(tmp), patched_server_db(conn):
            tmp_path, _ = server.build_export_bundle_file({"sampleIds": ["sample_keep"]})
            try:
                with server.zipfile.ZipFile(tmp_path, "r") as zf:
                    manifest = json.loads(zf.read("manifest.json").decode("utf-8"))
                    samples = json.loads(zf.read("domains/samples.json").decode("utf-8"))
                    projects = json.loads(zf.read("domains/projects.json").decode("utf-8"))
                    self.assertEqual(manifest["scope"], "selected-samples")
                    self.assertEqual([sample["id"] for sample in samples], ["sample_keep"])
                    self.assertEqual(projects, [])
            finally:
                tmp_path.unlink(missing_ok=True)

    def test_sample_archive_imports_into_empty_platform_without_projects(self):
        source = empty_state()
        source["sampleLibrary"]["categories"] = [{
            "id": "cat_source_archive",
            "name": "来源池",
            "samples": [{
                "id": "sample_archive_1",
                "sampleNo": "A001",
                "sn": "SN-ARCHIVE-1",
                "status": "闲置",
                "problemRecords": [{"description": "外部故障", "stage": "EVT"}],
                "photos": [],
            }],
        }]
        source["sampleLibrary"]["logs"] = [{
            "id": "event_archive_1",
            "sampleId": "sample_archive_1",
            "time": "2026-06-04T12:00:00+08:00",
            ...
            "result": "不通过",
            "taskStatus": "正常完成",
            "problemDescription": "屏幕亮线",
        }]
        old_previews = dict(server._IMPORT_PREVIEWS)
        server._IMPORT_PREVIEWS.clear()
        try:
            with tempfile.TemporaryDirectory() as tmp, patched_server_data_dirs(tmp):
                source_conn = state_conn(source)
                with patched_server_db(source_conn):
                    tmp_path, filename = server.build_sample_archive_file("sample_archive_1")
                    try:
                        self.assertTrue(filename.startswith("sample_archive_"))
                        archive_bytes = tmp_path.read_bytes()
                        with server.zipfile.ZipFile(tmp_path, "r") as zf:
                            names = set(zf.namelist())
                            self.assertIn("dossier.json", names)
                            self.assertIn("sample/info.json", names)
                            manifest = json.loads(zf.read("manifest.json").decode("utf-8"))
                            self.assertEqual(manifest["packageKind"], "sample-archive")
                            self.assertEqual(manifest["scope"], "selected-samples")
                    finally:
                        tmp_path.unlink(missing_ok=True)

                target_conn = state_conn(empty_state())
                with patched_server_db(target_conn):
                    headers, body = make_multipart(files=[
                        ("bundle", "sample_archive.zip", "application/zip", archive_bytes),
                    ])
                    preview = server.analyze_sample_archive(headers, body)
                    self.assertEqual(preview["packageKind"], "sample-archive")
                    result = server.commit_sample_archive({"previewId": preview["previewId"]})
                    self.assertTrue(result["ok"], result)
                    state, _, _ = server.get_state()
                    self.assertEqual(state["projects"], [])
                    categories = state["sampleLibrary"]["categories"]
                    self.assertEqual(categories[0]["name"], "外部导入样机")
                    self.assertEqual(categories[0]["samples"][0]["sn"], "SN-ARCHIVE-1")
                    self.assertEqual(len(state["sampleLibrary"]["logs"]), 2)
        finally:
            for preview_id in list(server._IMPORT_PREVIEWS):
                server._cleanup_preview_temp(preview_id)
            server._IMPORT_PREVIEWS.clear()
    ...
    def test_samples_page_filters_faults(self):
        ...
        data["sampleLibrary"] = {
            "categories": [{
                ...
                "samples": [
                    ...
                    {"id": "s2", "sampleNo": "A002", "sn": "SN002", "status": "闲置", "isReassembled": True, "problemRecords": [{"id": "p1", "description": "不开机"}]},
        ...
        reassembled_page = server.list_samples_page(conn, "c1", {
            "page": ["1"],
            "pageSize": ["10"],
            "reassembled": ["reassembled"],
        })
        normal_page = server.list_samples_page(conn, "c1", {
            "page": ["1"],
            "pageSize": ["10"],
            "reassembled": ["normal"],
        })

        self.assertEqual(reassembled_page["total"], 1)
        self.assertEqual(reassembled_page["items"][0]["id"], "s2")
        self.assertTrue(reassembled_page["items"][0]["isReassembled"])
        self.assertEqual(normal_page["total"], 2)
        self.assertEqual({item["id"] for item in normal_page["items"]}, {"s1", "s3"})

        keyword_reassembled_page = server.list_samples_page(conn, "c1", {
            "page": ["1"],
            "pageSize": ["10"],
            "keyword": ["SN002"],
            "reassembled": ["重组"],
        })
        keyword_normal_page = server.list_samples_page(conn, "c1", {
            "page": ["1"],
            "pageSize": ["10"],
            "keyword": ["SN002"],
            "reassembled": ["非重组"],
        ...
        self.assertEqual(keyword_reassembled_page["total"], 1)
        self.assertEqual(keyword_reassembled_page["items"][0]["id"], "s2")
        self.assertEqual(keyword_normal_page["total"], 0)
    ...
    def test_task_mutation_does_not_create_backups_or_use_global_db_lock(self):
        data = empty_state()
        data["projects"] = [{
            "id": "p1",
            "name": "项目A",
        ...
        self.assertTrue(ok, result)
        self.assertEqual(result["revision"], 2)
        self.assertFalse(hasattr(server, "BACKUP_DIR"))
        self.assertFalse(hasattr(server, "write_backup"))
    ...
    def test_commit_task_mutation_can_create_and_delete_task(self):
        ...
        with patched_server_db(conn):
            ok, result = server.commit_task_mutation({
                ...
                "stage": {"id": "st1", "projectId": "p1", "name": "阶段A", "progress": [{"id": "prog1", "testItem": "吞吐"}]},
                "task": {
                    "id": "task_new",
                    "projectId": "p1",
                    "stageId": "st1",
    ...
    def test_sample_history_page_is_aggregated_server_side(self):
        ...
        self.assertEqual(item["result"], "不通过")
        self.assertEqual(item["resultPhotos"][0]["result"], "不通过")
