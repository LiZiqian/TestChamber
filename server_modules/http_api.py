...
def handle_get(handler, ctx) -> None:
    ...
    if path == "/api/export-bundle":
        tmp_path = None
        try:
            tmp_path, filename = ctx.build_export_bundle_file()
    ...
    if path == "/api/bootstrap":
        try:
            ...
            handler._send_json({"ok": True, "revision": revision, "updated_at": updated_at, "data": data, "partial": True})
        except Exception as e:
            handler._send_json({"ok": False, "error": str(e)}, 500)
        return
    ...
    rel = path.lstrip("/")
    target = (ctx.ROOT_DIR / rel).resolve()
    if ctx.ROOT_DIR not in target.parents and target != ctx.ROOT_DIR:
