...
def run_server(
    *,
    ...
    parser.add_argument(
        "--data-dir",
        default=None,
        help=f"外置业务数据目录，默认使用平台目录同级 {default_data_dir}",
    )
    parser.add_argument(
        "--no-migrate-legacy-data",
        action="store_true",
        help="启动时不自动复制旧 data/ 与 backups/ 到外置数据目录",
    )
    ...
    if migration_report.migrated:
        print(f"已复制旧数据到外置数据目录，文件 {migration_report.copied_files} 个，目录 {migration_report.copied_dirs} 个")
