...
boundary between platform code and user data so server.py can stay focused on
HTTP/API orchestration while data-root rules remain testable in isolation.
"""
...
def validate_external_data_root(data_root: Path, platform_root: Path) -> None:
    if path_is_inside(data_root, platform_root):
        raise ValueError(
            f"数据目录不能位于平台代码目录内部: {data_root}。"
            f"请使用代码目录外的路径，例如 {default_data_dir(platform_root)}。"
        )
...
def has_legacy_data(platform_root: Path) -> bool:
    platform_root = Path(platform_root).resolve()
    return (platform_root / "data").exists() or (platform_root / "backups").exists()
...
def data_root_has_business_data(paths: RuntimePaths) -> bool:
    ...
    if paths.backup_dir.exists():
        for item in paths.backup_dir.iterdir():
...
def copy_tree_without_loss_detailed(src: Path, dst: Path) -> CopyTreeReport:
    if not src.exists():
...
def prepare_runtime_paths(
    ...
    *,
    ...
    validate_external_data_root(target_root, platform_root)
    paths = build_runtime_paths(target_root)
    legacy_data_dir = platform_root / "data"
    legacy_backup_dir = platform_root / "backups"

    if not migrate_legacy or not has_legacy_data(platform_root):
        ensure_runtime_dirs(paths, platform_root=platform_root)
        return paths, DataRootMigrationReport(paths.data_dir, False, legacy_data_dir, legacy_backup_dir, 0, 0)
    ...
    try:
        ...
        report = copy_tree_without_loss_detailed(legacy_data_dir, staging)
        ...
        report = copy_tree_without_loss_detailed(legacy_backup_dir, staging_paths.backup_dir)
        copied_files += report.copied_files
        copied_dirs += report.copied_dirs
        copied_bytes += report.copied_bytes
        verified_files += report.verified_files
        ensure_runtime_dirs(
            ...
            migration={
                ...
                "legacyBackupDir": str(legacy_backup_dir),
        ...
        if paths.data_dir.exists():
            if not directory_is_empty(paths.data_dir):
