...
from server_modules import mutation_summary, record_writers, sample_assets, task_mutation_rules
...
class MutationServiceContext:
    ...
    should_backup: Callable[[str, int], bool]
    write_compact_backup_snapshot: Callable[[int], None]
...
def _maybe_write_compact_backup(ctx: MutationServiceContext, action: str, revision: int, *, warn_label: str) -> None:
    if not ctx.should_backup(action, revision):
        return
    try:
        ctx.write_compact_backup_snapshot(revision)
...
def commit_task_mutation(ctx: MutationServiceContext, payload: dict, client_ip: str) -> tuple[bool, dict]:
    ...
    _maybe_write_compact_backup(ctx, action, new_revision, warn_label="写入任务增量备份失败")
...
def commit_task_batch_mutation(ctx: MutationServiceContext, payload: dict, client_ip: str) -> tuple[bool, dict]:
    ...
    _maybe_write_compact_backup(ctx, action, new_revision, warn_label="写入批量任务增量备份失败")
