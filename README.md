...
```mermaid
flowchart TB
    classDef project fill:#eff6ff,stroke:#60a5fa,stroke-width:2px,color:#0f172a,font-size:15px,font-weight:700;
    classDef sample fill:#f0fdf4,stroke:#86efac,stroke-width:2px,color:#0f172a,font-size:15px,font-weight:700;
    classDef task fill:#fffbeb,stroke:#fbbf24,stroke-width:2px,color:#0f172a,font-size:15px,font-weight:700;
    classDef result fill:#fff7ed,stroke:#fb923c,stroke-width:2px,color:#0f172a,font-size:15px,font-weight:700;
    classDef archive fill:#f8fafc,stroke:#94a3b8,stroke-width:2px,color:#0f172a,font-size:15px,font-weight:700;
    classDef muted fill:#ffffff,stroke:#cbd5e1,stroke-width:1px,color:#475569,font-size:12px,font-weight:400;

    subgraph ProjectFlow["项目链路"]
        direction TB
        Project["项目管理"]:::project
        ProjectDetail["阶段 / 方案 / 人员"]:::muted
        Sheet["测试任务工作表"]:::project
        SheetDetail["按阶段生成、筛选和跟踪任务"]:::muted
        Project --> ProjectDetail --> Sheet --> SheetDetail
    end

    subgraph SampleFlow["样机链路"]
        direction TB
        SampleArchive["样机档案库"]:::sample
        Pool["样机池 A / B"]:::muted
        SampleCard["样机卡片"]:::sample
        SampleDetail["详情 / 履历 / 图片 / CT 数据"]:::muted
        SampleArchive --> Pool --> SampleCard --> SampleDetail
    end

    subgraph ExecuteFlow["执行闭环"]
        direction TB
        Task["任务执行"]:::task
        TaskDetail["执行人 / 分配样机 / 启动 / 阻塞"]:::muted
        Result["结果录入"]:::result
        ResultDetail["测试结论 / 故障照片 / 分析照片 / 样机去向"]:::muted
        Archive["履历与迁移"]:::archive
        ArchiveDetail["样机履历 / 完整数据包"]:::muted
        Task --> TaskDetail --> Result --> ResultDetail --> Archive --> ArchiveDetail
    end

    SheetDetail --> Task
    SampleDetail --> Task

    style ProjectFlow fill:#f8fbff,stroke:#bfdbfe,stroke-width:1px,color:#f8fafc
    style SampleFlow fill:#f7fef9,stroke:#bbf7d0,stroke-width:1px,color:#f8fafc
    style ExecuteFlow fill:#fffdf5,stroke:#fde68a,stroke-width:1px,color:#f8fafc
```
