...
BACKUP_DIR = _RUNTIME_PATHS.backup_dir
...
def validate_external_data_root(path: Path) -> None:
    runtime_paths.validate_external_data_root(path, ROOT_DIR)


def _apply_runtime_paths(paths: runtime_paths.RuntimePaths) -> None:
    global _RUNTIME_PATHS, DATA_DIR, SAMPLE_DATA_DIR, BACKUP_DIR, IMPORT_PREVIEW_DIR, EXPORT_DIR, DB_PATH, DEPLOYMENT_FILE
...
BACKUP_INTERVAL_SECONDS = backup_service.BACKUP_INTERVAL_SECONDS
BACKUP_REVISION_GAP = backup_service.BACKUP_REVISION_GAP
BACKUP_MIN_INTERVAL_SECONDS = backup_service.BACKUP_MIN_INTERVAL_SECONDS
IMPORTANT_BACKUP_MIN_INTERVAL_SECONDS = backup_service.IMPORTANT_BACKUP_MIN_INTERVAL_SECONDS
BACKUPS_PER_HOUR_KEEP = backup_service.BACKUPS_PER_HOUR_KEEP
MAX_BACKUPS_KEEP = backup_service.MAX_BACKUPS_KEEP


def _should_backup(action: str, revision: int, *, is_important: bool = False) -> bool:
    """判断是否应该生成一份 backup JSON 快照。"""
    return backup_service.should_backup(revision, is_important=is_important)


def write_backup(data: dict, revision: int) -> None:
    backup_service.write_backup(BACKUP_DIR, data, revision, json_dumps)
...
def prune_backups() -> None:
    backup_service.prune_backups(BACKUP_DIR)
...
def prepare_export_bundle_parts() -> tuple[dict, dict, dict[str, str], dict, str]:
    return bundle_preview_service.prepare_export_bundle_parts(_bundle_preview_context())
...
def build_export_bundle_file() -> tuple[Path, str]:
    return bundle_preview_service.build_export_bundle_file(_bundle_preview_context())
...
def _state_persistence_context() -> state_persistence.StatePersistenceContext:
    return state_persistence.StatePersistenceContext(
        ...
        sync_sample_library=sync_sample_library,
        split_state_for_storage=split_state_for_storage,
        should_backup=lambda action, revision, is_important=False: _should_backup(action, revision, is_important=is_important),
        write_backup=write_backup,
        load_sample_photos=load_sample_photos,
    )
...
def _mutation_service_context() -> mutation_services.MutationServiceContext:
    return mutation_services.MutationServiceContext(
        write_db_connection=write_db_connection,
        now_iso=now_iso,
        should_backup=lambda action, revision: _should_backup(action, revision),
        write_compact_backup_snapshot=write_compact_backup_snapshot,
