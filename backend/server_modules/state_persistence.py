...
from server_modules import record_writers, status_normalization
...
def save_state(
    ...
    new_data = status_normalization.normalize_state_payload(new_data)
...
def commit_data_mutation(ctx: StatePersistenceContext, conn: sqlite3.Connection, data: dict, action: str, remark: str, client_ip: str) -> dict:
    ...
    new_revision = current_revision + 1
    updated_at = ctx.now_iso()
    data = status_normalization.normalize_state_payload(data)
